...
def get_team_info():
    ...
    teams = driver.find_elements_by_xpath(
        "/html/body/div[1]/div/div/div[4]/main/div[3]/section[1]/a")
    # teams = container.find_elements_by_tag_name('div')
    print(name, len(teams))
    for team in teams:
        try:
            location = team.find_elements_by_xpath(
                "./div/div/div[2]/div[3]/div/*")[-1].text
        except:
            location = "None"
        day = team.find_element_by_xpath(
            "./div/div/div[1]/div[2]").text
        month = team.find_element_by_xpath(
            "./div/div/div[1]/div[1]").text
        price = team.find_element_by_xpath(
            "./div/div/div[2]/div[1]").text
        title = team.find_element_by_xpath(
